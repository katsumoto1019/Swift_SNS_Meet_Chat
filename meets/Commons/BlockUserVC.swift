...
class BlockUserVC: BaseVC {
    ...
    @IBOutlet weak var tbv_blockusers: UITableView!
    var ds_blockusers = [NotiModel]()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = Messages.FAVORITELIST
        tbv_blockusers.tableFooterView = UIView()
    }
    ...
    func getDataSource()  {
        ...
        for i in 0 ... 9{
            ...
            self.ds_blockusers.append(NotiModel(id: "\(i)", senderInfo: userone, time: TestData.postTime[i], content: TestData.postContent[i]))
            if num == 10{
                self.tbv_blockusers.reloadData()
            }
        }*/
            ...
            if isSuccess{
                ...
                if let notificationinfo = notification_info{
                    if notificationinfo.count != 0{
                        ...
                        //まだ通知はありません。
                        self.tbv_blockusers.reloadData()
                        self.showToast("ブロックされたユーザーはいません。")
                    }
                }
            }
        }
    ...
    @IBAction func deleteBtnClickekd(_ sender: Any) {
        ...
        ApiManager.removeBlockUser(user_id: "\(self.ds_blockusers[indexpath].senderInfo.id ?? 0)"){ (isSuccess, data) in
            if isSuccess{
                self.getDataSource()
            }else{
...
extension BlockUserVC: UITableViewDataSource, UITableViewDelegate{
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return self.ds_blockusers.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tbv_blockusers?.dequeueReusableCell(withIdentifier: "FavoriteCell", for:indexPath) as! FavoriteCell
        cell.selectionStyle = .none
        cell.entity = ds_blockusers[indexPath.section]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        //self.gotoGeneralProfileVC(self.ds_blockusers[indexPath.section].senderInfo, isLikeUser: false, chattingOption: .fromNotiVC)
    }
